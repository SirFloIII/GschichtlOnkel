...
Each region also has a set of (yet) unassigned points, which determine where the region might grow in the next iteration, called the halo.
...
\For{each levelset in bottom to top order}
    \State Add points to regions if they are in the halo of a region
    \If{an added point is in the halo of different region}
        \State Find the union of the halos of the involved regions
        \State Find the connected components thereof
        \State Assign these to the regions in an arbitrary way
	\EndIf
    \If{an added point splits the halo of the region in two}
        \State Reduce the halo the region to one component
        \For{each other component}
            \State Create a new region containing the component as halo
		\EndFor
    \EndIf
...
\section{Conclusion}
In this paper we have shown that slope regions in high dimensions ($n\geq 3$) do not have the same critical point properties well-established in 2D.
