...
Florian Bogner was to train a neural network as a recognizer for saddle points. He wrote a framework to generate test data through iterated concatenation of well-known functions. This data was to be searched for saddle points by Palmrich's algorithm and then be used as training data for the network. Since the saddle detector proved unsatisfactory, this approach was also shelved, and energy was directed to the joint development of BP.

Most of the theory behind BP as well as the actual code was conceived in direct interaction between both authors. Some tidbits might be attributed to one of them, such as Bogner's idea to swap halo components between regions, his proof of theorem \ref{slope_iff_conn_lvlsets},
...
% individual contributions F': theorem, border swap, interactive plot, ... A: halo data structure, moving along iso surfaces during search, ...
% dismissed approach of local regression & neural network
