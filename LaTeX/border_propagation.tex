...
\begin{thm}
Let $L$ be a connected linear subset of $G$. $S := \bigcup_{l \in L} l$ is a slope region with the additional property that its internal borders are along levelsets.

\emph{Proof:} Also follows from Theorem \ref{slope_iff_conn_lvlsets}.\hfill $\Box$
\end{thm}

Thus, decompositions of a Reeb Graph $G$ into connected linear subsets of $G$ are isomorphic to a Slope Region Decompositions with borders along levelsets, which are exactly the kind of slope region decompositions which BP produces.
