...
Given $x, y \in A$ we have to find a monotonic path $\gamma$. Since $A$ is path-connected, there exists an (not necessarily monotonic) path $\gamma_0$ from $x$ to $y$. If it is monotonic, we win. Else, we leave it as an exercise for the reader.
